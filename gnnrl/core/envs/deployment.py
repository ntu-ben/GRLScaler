...
def _get_default_service_graph(namespace: str):
    """Return a default service graph when Kiali is not available."""
    if namespace == "onlineboutique":
        # Default OnlineBoutique service topology
        services = ["recommendationservice", "productcatalogservice", "cartservice", 
                   "adservice", "paymentservice", "shippingservice", "currencyservice",
                   "redis-cart", "checkoutservice", "frontend", "emailservice"]
        
        # Create basic service connections (simplified topology)
        edge_list = [
            (9, 0), (9, 1), (9, 2), (9, 3),  # frontend -> other services
            (2, 7),  # cartservice -> redis-cart  
            (8, 4), (8, 5),  # checkoutservice -> payment, shipping
            (0, 1),  # recommendation -> catalog
        ]
...
def get_kiali_service_graph(namespace: str, graph_type: str = "workload"):
    ...
    # Try multiple API endpoint formats with correct Golang duration format
    api_urls = [
        f"{KIALI_URL}/api/namespaces/graph?namespaces={namespace}&duration=5m&graphType={graph_type}",
        f"{KIALI_URL}/api/namespaces/{namespace}/graph?duration=5m&graphType={graph_type}",
        f"{KIALI_URL}/api/graph?namespaces={namespace}&duration=10m&graphType={graph_type}"
    ]
    ...
    for url in api_urls:
        try:
            ...
            logging.debug(f"Kiali API attempt failed for {url}: %s", e)
            continue
    else:
        logging.warning("All Kiali API endpoints failed, using default service graph")
        return _get_default_service_graph(namespace)
    ...
    def update_obs_k8s(self):
        ...
        for p in self.pod_names:
            query_cpu = 'sum(irate(container_cpu_usage_seconds_total{namespace=' \
                        '"' + self.namespace + '", pod="' + p + '"}[1m])) by (pod)'
            ...
            if self.name == 'redis-master':
                query_duration = 'sum(irate(redis_commands_duration_seconds_total[1m]))'
                query_processed = 'sum(irate(redis_commands_processed_total[1m]))'
                redis_duration = 0
                redis_processed = 0
