...
# Constants
MAX_CPU = 1000  # cpu in m
MAX_MEM = 1000  # memory in MiB
...
ARIMA_CPU = "forecast-arima-cpu-usage"
ARIMA_MEM = "forecast-arima-memory-usage"
...
def convert_to_mega_memory(value):
    last_two = value[-2:]
    size = len(value)
    new_value = int(float(value[:size - 2]))
    if last_two == "Ki":
...
class DeploymentStatus:  # Deployment Status (Workload)
    ...
    def update_obs_k8s(self):
        self.pod_names = []
        pods = self.v1.list_namespaced_pod(namespace=self.namespace, label_selector='app=' + self.name)
        for p in pods.items:
        ...
        # Update Forecast
        # forecast-arima-cpu-usage: 1817.627m
        # forecast-arima-memory-usage: 6.431Mi
        self.cpu_forecast = convert_to_milli_cpu(self.deployment_object.metadata.annotations[ARIMA_CPU])
        self.mem_forecast = convert_to_mega_memory(self.deployment_object.metadata.annotations[ARIMA_MEM])
        logging.info("[Deployment] Redis forecast - CPU: {} - MEM: {} |".format(self.cpu_forecast, self.mem_forecast))
